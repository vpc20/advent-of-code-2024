...
for e in arr:
    # print(e)
    increasing = all(i < j for i, j in zip(e, e[1:]))
    decreasing = all(i > j for i, j in zip(e, e[1:]))
    diff_ok = all(1 <= abs(i - j) <= 3 for i, j in zip(e, e[1:]))
    if (increasing or decreasing) and diff_ok:
        ctr += 1
print(ctr)
# 236
...
for e in arr:
    ...
    for i in range(len(e)):
        ex = e[:i] + e[i + 1:]
        increasing = all(i < j for i, j in zip(ex, ex[1:]))
        decreasing = all(i > j for i, j in zip(ex, ex[1:]))
        diff_ok = all(1 <= abs(i - j) <= 3 for i, j in zip(ex, ex[1:]))
        if (increasing or decreasing) and diff_ok:
            ctr += 1
            break
print(ctr)
